...
import com.massttr.user.databinding.ActivityChatBinding
import com.massttr.user.databinding.DialogPhotoVideoPickerBinding
...
class ChatActivity : BaseActivity<ActivityChatBinding>(R.layout.activity_chat),
    ...
    companion object {
        ...
        private const val REQ_CHANGE_PRICE = 77
    }
    ...
    private fun init() {
        ...
        chatAdapter.setItemClickListener { view, i, chatMessage ->
            when (view.id) {
                R.id.ivMyImage,
                R.id.ivOtherImage->{
                     browse(chatMessage.message)
                }
                R.id.ivMyVideo,
                R.id.ivOtherVideo -> {
                    startActivity<VideoViewActivity>("url" to chatMessage.message)
                   // browse(chatMessage.message)
                }
            }
        }
    ...
    private fun setUpUI() {
        ...
        notificationChatMessage?.let {
            isChatCreate = true
            SocketService.getInstance()?.sendCreateChat(it.sender_id, it.job_id)
        }
    }

    private fun setupInbox() {
        inbox?.let { inbox ->
            if (SocketService.getSocketInstance()?.connected() == true) {
                SocketService.getInstance()?.run {
            ...
            binding.ivCall.isVisible = inbox.job_status_id==2||inbox.job_status_id==3||inbox.job_status_id==4
        }
    }

    private fun initListener() {
        binding.run {
            ivLockPrice.setOnClickListener(this@ChatActivity)
            ...
            srlChat.setOnRefreshListener {
                if (SocketService.getSocketInstance()?.connected() == true) {
                    offset++
                    inbox?.let { SocketService.getInstance()?.sendGetMessage(it.chat_id, offset) }
                } else {
                    handler.postDelayed({
        ...
        EventBus.subscribe<Bundle>(listSubscription) {
            ...
            if (it.containsKey(BUS_EVENT_MESSAGES_LIST)) {
               // handler.postDelayed({ updateLoaderUI(false) }, 50)
                 runOnUiThread {
                     ...
                     val chatMessages: ArrayList<ChatMessage>? = it.getParcelableArrayList(BUS_EVENT_MESSAGES_LIST)
                    ...
                    if ((binding.rvChat.layoutManager as LinearLayoutManager).findLastVisibleItemPosition() == chatAdapter.itemCount - 1) {
                        if (newChatMessage.sender_id != pref.userInfo?.id) {
                            ...
                            inbox?.chat_id?.let { chatId ->
                                SocketService.getInstance()?.sendReadMessage(chatId, newChatMessage.m_id)
                                val list = ArrayList(pref.unreadChatMessages.orEmpty())
                                val index =
                        ...
                        val bundle = Bundle()
                        bundle.putParcelable(BUS_EVENT_READ_ALL_MESSAGES_LOCALLY, InboxItem(newChatMessage.chat_id))
            ...
            if (it.containsKey(BUS_EVENT_DELIVERED_MESSAGE)) {
                ...
                if (message?.chat_id == inbox?.chat_id) {
                    val index = chatAdapter.displayList.indexOfLast { chatMessage -> chatMessage.m_id == message?.m_id }
                    if (index != -1) {
    ...
    private fun setUpObserver() {
        viewModel.run {
            ...
            uploadChatMedia.observe(this@ChatActivity) {
                inbox?.let { inbox ->
                    SocketService.getInstance()?.sendChatMessage(
                        inbox.chat_user_id,
                        it.media_path, if (isImageUpload) MessageType.PHOTO.type else MessageType.VIDEO.type,
                        inbox.chat_id,
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.ivLockPrice -> {
                startActivityForResult<EditTaskPriceActivity>(
                    REQ_CHANGE_PRICE,
                    EditTaskPriceActivity.JOB_ID to inbox?.job_id,
                    EditTaskPriceActivity.PRICE to inbox?.job_price
                )
            }
            R.id.ivAttachOpen -> {
                onAllowStoragePermissionWithPermissionCheck()
            }
            R.id.ivCall -> {
            ...
            R.id.ivSend -> {
                if (binding.etMessage.text.toString().isNotEmpty()) {
                    inbox?.let { inbox ->
                        SocketService.getInstance()?.sendChatMessage(
                            inbox.chat_user_id,
                            binding.etMessage.text.toString(), MessageType.TEXT.type,
                            inbox.chat_id,
                            pref.userInfo?.profile_picture.orEmpty(),
                            pref.userInfo?.full_name.orEmpty(), inbox.job_id
                        )
    ...
    private fun attachAlert() {
        ...
        dialogBinding.alert.setOnClickListener {
            binding.ivAttachOpen.isVisible = true
            dialog.dismiss()
        }
...
//            resultImageLauncher.launch(Intent.createChooser(intent, "Select Picture"))
//            dialog.dismiss()
...
//            resultVideoLauncher.launch(Intent.createChooser(intent, "Select Video"))
    ...
    private fun showMoveDownButton(show: Boolean) {
        if (show) {
            binding.ivMoveDown.isVisible = true
        } else {
            binding.ivMoveDown.isVisible = false
    ...
    private fun readAllMessages() {
        inbox?.let { inbox ->
            ...
            if (!allMessageSeen) {
                ...
                SocketService.getInstance()?.sendReadAllMessages(inbox.chat_id)
    ...
    private val typingRunnable = object : Runnable {
        override fun run() {
            ...
            inbox?.let {
                SocketService.getInstance()?.sendTypingStatus(isTyping, it.chat_id)
            }
...
//    }
//
//    var resultVideoLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
//        if (result.resultCode == Activity.RESULT_OK) {
...
//    }+
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == REQ_CHANGE_PRICE) {
                val changedPrice = data?.getStringExtra(EditTaskPriceActivity.RESULT_CHANGED_PRICE)

                if (changedPrice?.isNotEmpty() == true) {
                    inbox?.let { inbox ->
                        SocketService.getInstance()?.sendChatMessage(
                            inbox.chat_user_id,
                            changedPrice, MessageType.CHANGE_PRICE.type,
                            inbox.chat_id,
                            pref.userInfo?.profile_picture.orEmpty(),
                            pref.userInfo?.full_name.orEmpty(), inbox.job_id
                        )
