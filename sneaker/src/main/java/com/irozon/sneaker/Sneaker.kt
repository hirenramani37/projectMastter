...
class Sneaker(private var context: Context) : View.OnClickListener {
    ...
    private var mMarginLeft = DEFAULT_VALUE
    private var mMarginTop = DEFAULT_VALUE
    private var mMarginRight = DEFAULT_VALUE
    private var mMarginBottom = DEFAULT_VALUE
    ...
    private fun setTargetView(targetView: Activity) {
        this.targetView = targetView.window?.decorView as ViewGroup
    }
    ...
    fun setCornerRadius(radius: Int): Sneaker {
        setCornerRadius(radius, DEFAULT_VALUE, DEFAULT_VALUE, DEFAULT_VALUE, DEFAULT_VALUE)
        return this
    }
    ...
    /**
     * Shows sneaker with custom colors
     *
     * @param backgroundColors Color resource for sneaker background color in gradient
     */
    fun sneak(backgroundColors: IntArray) {
    ...
    private fun sneakView() {
        ...
        targetView?.let {
            val layoutParams = LinearLayout.LayoutParams(
                ...
                if (mHeight == DEFAULT_VALUE)
                    Utils.convertDpToPixel(context, 56f).toInt()
                else
                    Utils.convertDpToPixel(context, mHeight.toFloat()).toInt()
            )
            if (mMarginLeft != DEFAULT_VALUE && mMarginTop != DEFAULT_VALUE && mMarginRight != DEFAULT_VALUE && mMarginBottom != DEFAULT_VALUE) {
                val marginLeft = Utils.convertDpToPixel(context, mMarginLeft.toFloat()).toInt()
                val marginTop = Utils.convertDpToPixel(context, mMarginTop.toFloat()).toInt()
                val marginRight = Utils.convertDpToPixel(context, mMarginRight.toFloat()).toInt()
                val marginBottom = Utils.convertDpToPixel(context, mMarginBottom.toFloat()).toInt()

                val idStatusBarHeight =
                    context.resources.getIdentifier("status_bar_height", "dimen", "android")
                val height = context.resources.getDimensionPixelSize(idStatusBarHeight)

                layoutParams.setMargins(
                    marginLeft,
                    /*if (Utils.getStatusBarHeight(it) == 0) 56 + marginTop else Utils.getStatusBarHeight(
                        it
                    ) + marginTop,*/
                    height + marginTop,
                    marginRight,
            ...
            with(sneakerView) {
                ...
                setPadding(
                    Utils.convertDpToPixel(context, 1f).toInt(),
                    0,
                    Utils.convertDpToPixel(context, 1f).toInt(),
                    0
                )
                if (mBackgroundColors.isEmpty())
                    setBackground(mBackgroundColor, mCornerRadius)
                else
                    setBackground(mBackgroundColors, mCornerRadius)
                setIcon(
                    mIconDrawable,
                    mIsCircular,
                    Utils.convertDpToPixel(context, mIconSize.toFloat()).toInt(),
                    mIconColorFilterColor
                )
